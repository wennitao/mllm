...
" c3 += b0*w.sc; c3 += b1*w.sd; c3 += b2*w.se; c3 += b3*w.sf;\n"
" }\n"
" #define BSEMIT(NN, CV) { "" const int ql = gx_4 + (NN); "" if (ql < BQ) { "" const int q_pos = (Skv - Sq) + qb * BQ + ql; "" float8 v = (CV) * scale; half8 hv; "" hv.s0 = (half)((blk_id<0 || (causal && (kg0+0) > q_pos)) ? -INFINITY : clamp(v.s0,-TP_CLAMP,TP_CLAMP)); "" hv.s1 = (half)((blk_id<0 || (causal && (kg0+1) > q_pos)) ? -INFINITY : clamp(v.s1,-TP_CLAMP,TP_CLAMP)); "" hv.s2 = (half)((blk_id<0 || (causal && (kg0+2) > q_pos)) ? -INFINITY : clamp(v.s2,-TP_CLAMP,TP_CLAMP)); "" hv.s3 = (half)((blk_id<0 || (causal && (kg0+3) > q_pos)) ? -INFINITY : clamp(v.s3,-TP_CLAMP,TP_CLAMP)); "" hv.s4 = (half)((blk_id<0 || (causal && (kg0+4) > q_pos)) ? -INFINITY : clamp(v.s4,-TP_CLAMP,TP_CLAMP)); "" hv.s5 = (half)((blk_id<0 || (causal && (kg0+5) > q_pos)) ? -INFINITY : clamp(v.s5,-TP_CLAMP,TP_CLAMP)); "" hv.s6 = (half)((blk_id<0 || (causal && (kg0+6) > q_pos)) ? -INFINITY : clamp(v.s6,-TP_CLAMP,TP_CLAMP)); "" hv.s7 = (half)((blk_id<0 || (causal && (kg0+7) > q_pos)) ? -INFINITY : clamp(v.s7,-TP_CLAMP,TP_CLAMP)); "" vstore8(hv, 0, Sh + (long)ql * sel + gy_8); "" } }\n"
