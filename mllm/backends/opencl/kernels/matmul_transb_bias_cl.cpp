...
" #define V5_STEP(KK, MASK, SH) { "" B.s0123 = read_imageh(acts, gy*2 + (i+KK)*M_4); "" B.s4567 = read_imageh(acts, gy*2 + (i+KK)*M_4 + 1); "" dq.s0 = (((bits.s0 & (MASK)) >> SH) - 8) * scale.s0; "" dq.s1 = (((bits.s1 & (MASK)) >> SH) - 8) * scale.s1; "" dq.s2 = (((bits.s2 & (MASK)) >> SH) - 8) * scale.s2; "" dq.s3 = (((bits.s3 & (MASK)) >> SH) - 8) * scale.s3; "" c0 += B * dq.s0; c1 += B * dq.s1; c2 += B * dq.s2; c3 += B * dq.s3; "" }\n"
" V5_STEP(0,0x000F,0); V5_STEP(1,0x00F0,4);\n"
" V5_STEP(2,0x0F00,8); V5_STEP(3,0xF000,12);\n"
" #undef V5_STEP\n"
" }\n"
" #define WRITE_M(SM) "" vstore4((half4)(c0.s##SM, c1.s##SM, c2.s##SM, c3.s##SM), 0, "" dst+(long)((gy << 3)+(SM))*N+gx_4);\n"
