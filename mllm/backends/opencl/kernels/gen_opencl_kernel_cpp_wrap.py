...
def opencl_codegen():
    ...
    opencl_source_hpp += "namespace mllm::opencl { \n"
    ...
    for file_name_all in os.listdir(cl_kernel_dir):
        ...
        if file_path[-3:] == ".cl":
            with open(file_path, "r", encoding = 'utf-8') as f:
                ...
                opencl_source_map = "#include \"a_opencl_source_map.hpp\" \n"
                opencl_source_map += "namespace mllm::opencl { \n"
                if file_name[-4:] == "_buf":
                    opencl_source_map += "#ifndef MLLM_OPENCL_BUFFER_CLOSED\n"
                    opencl_source_hpp += "#ifndef MLLM_OPENCL_BUFFER_CLOSED\n"
                    opencl_source_map_hpp += "#ifndef MLLM_OPENCL_BUFFER_CLOSED\n"
                if file_name[-13:] == "_subgroup_buf":
                    opencl_source_map += "#ifdef MLLM_SUPPORT_INTEL_SUBGROUP\n"
                    opencl_source_hpp += "#ifdef MLLM_SUPPORT_INTEL_SUBGROUP\n"
                    opencl_source_map_hpp += "#ifdef MLLM_SUPPORT_INTEL_SUBGROUP\n"
            ...
            #write into cpp
            output_path = file_path[:-3] + "_cl.cpp"
            with open(output_path, "w") as cpp_file:
                cpp_file.write(opencl_source_map)
            os.system("clang-format -i " + output_path)

    #write into hpp
    opencl_source_map_hpp += "};\n"
    opencl_source_map_hpp += "} // namespace mllm::opencl\n"
    opencl_md5_map_hpp += "};\n"
    with open("a_opencl_source_map.hpp", "w") as w_file:
        w_file.write(opencl_source_hpp)
        w_file.write(opencl_source_map_hpp)
        w_file.write(opencl_md5_map_hpp)
    os.system("clang-format -i a_opencl_source_map.hpp")
