...
//
//   acts (image1d_buffer of float4): [K, M] float (transposed; 8 tokens/thread)
//   weights (__global float*):       [K/4, N, 4] float  (4 K-values contiguous)
...
// Weights: [K/4, N, 4] float. For K-group g=i/4 and the 4 output channels this
...
__kernel void fp32_gemm_vc(
    __read_only image1d_buffer_t acts,    // [K, M] float (1d image of float4)
    __global const float*         weights,  // [K/4, N, 4] float
  ...
  float8 c0 = (float8)((float)0), c1 = (float8)((float)0);
  float8 c2 = (float8)((float)0), c3 = (float8)((float)0);
  float8 B0, B1, B2, B3;

  for (int i = 0; i < K; i += 4) {
    // 8 tokens at each of K=i..i+3 (two float4 reads per K).
    B0.s0123 = read_imagef(acts, gy * 2 + (i + 0) * M_4);
    B0.s4567 = read_imagef(acts, gy * 2 + (i + 0) * M_4 + 1);
    B1.s0123 = read_imagef(acts, gy * 2 + (i + 1) * M_4);
    B1.s4567 = read_imagef(acts, gy * 2 + (i + 1) * M_4 + 1);
    B2.s0123 = read_imagef(acts, gy * 2 + (i + 2) * M_4);
    B2.s4567 = read_imagef(acts, gy * 2 + (i + 2) * M_4 + 1);
    B3.s0123 = read_imagef(acts, gy * 2 + (i + 3) * M_4);
    B3.s4567 = read_imagef(acts, gy * 2 + (i + 3) * M_4 + 1);

    // ONE load: 4 channels × 4 K-values = 16 halfs.
    float16 w = vload16(0, weights + ((long)(i >> 2) * N + gx_4) * 4);
...
void pack_W_NK_to_K4N4(int N, int K, const float* W_NK, float* W_p) {
  const int K_4 = K / 4;
  for (int g = 0; g < K_4; ++g)
    for (int n = 0; n < N; ++n)
...
MLLM_MAIN({
  ...
  fmt::print("[opencl-fp32-vc] init; local={{{}, {}}}\n", lx, ly);
  ...
  cl_kernel k_gemm = OpenCLLoader::instance().clCreateKernel(prog, "fp32_gemm_vc", &err);
  CL_CHECK(err);
  ...
  fmt::print("\n=== OpenCL fp32-GEMM variant C (q4_0-style packed loads, reps={}) ===\n", reps);
  ...
  for (auto sh : shapes) {
    ...
    for (auto& x : A_MK) x = (float)dist(rng);
    for (auto& x : W_NK) x = (float)dist(rng);
    ...
    std::vector<float> W_p((size_t)N * K);
    pack_W_NK_to_K4N4(N, K, W_NK.data(), W_p.data());
    cl_mem d_W = create_and_upload(ctx, q, W_p.data(), W_p.size() * 4, CL_MEM_READ_ONLY);
